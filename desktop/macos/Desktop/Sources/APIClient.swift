...
actor APIClient {
  ...
  func buildHeaders(
    ...
    if includeBYOK, APIKeyService.isByokActive {
      ...
      let canAttachAllBYOK = await MainActor.run {
        snapshot.allSatisfy { item in
          health.canUseBYOK(provider: item.key, fingerprint: item.value.fingerprint)
        }
      }
      if canAttachAllBYOK {
        ...
        log("CredentialHealth: context=build_headers failure_class=byok_invalid_suppressed")
      }
    }
