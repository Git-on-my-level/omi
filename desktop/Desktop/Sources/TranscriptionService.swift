...
class TranscriptionService {
  ...
  private static func sanitizedContextKeywords(_ keywords: [String]) -> [String] {
    let stopWords: Set<String> = [
      "about", "after", "again", "all", "also", "and", "app", "are", "ask", "back", "browser",
      "but", "can",
      "chat", "code", "done", "each", "for", "from", "get", "has", "have", "help", "here", "home",
      "how",
      "into", "just", "like", "means", "more", "next", "not", "now", "open", "question", "read",
      "right",
      "said", "screen", "sent", "show", "some", "task", "test", "text", "that", "the", "this",
      "time",
  ...
  init(language: String = "en", mode: StreamingMode = .conversation, contextKeywords: [String] = [])
    throws
  {
  ...
  func cleanupAndReconnect() {
    ...
    log(
      "TranscriptionService: Reconnecting in \(delay)s (attempt \(reconnectAttempts), pre-connect failure)"
    )
...
extension TranscriptionService {
  ...
  static func batchTranscribe(
    ...
    if !sanitizedKeywords.isEmpty {
      queryItems.append(
        URLQueryItem(name: "keywords", value: sanitizedKeywords.joined(separator: ",")))
    }
    components.queryItems = queryItems
    ...
    guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
      ...
      logError(
        "TranscriptionService: Batch transcription failed with status \(statusCode): \(body)",
        error: nil)
      if statusCode == 413 {
