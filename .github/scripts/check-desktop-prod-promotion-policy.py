...
def main() -> int:
    ...
    require("\n  schedule:", text, "scheduled reconciler must catch missed release edits (GitHub does not reliably emit Actions for release edits)")
    ...
    require("EVENT_NAME: ${{ github.event_name }}", text, "workflow must distinguish release events from scheduled reconciliation and manual retries")
    require('if [ "$EVENT_NAME" = "release" ]; then', text, "release events must use the edited/published release")
    require('elif [ "$EVENT_NAME" = "schedule" ]', text, "scheduled reconciler must scan for the newest stable release")
    ...
    require("git merge-base --is-ancestor", text, "prod deploys must use roll-forward guard to prevent downgrades")
